...
\begin{teorema}
    ...
    que $1 \leq s_i \leq |V(H_i)|$ y $\sum\limits_{i=1}^{m}  s_i = k$, se tiene
    que $F_{s_1}(H_1) \square \cdots \square F_{s_m}(H_m)$ es una subgr\'afica
    inducida de $F_k(G)$.
\end{teorema}
...
\begin{proof}
    ...
    V(H_m)|=m \}$. Observamos que, debido a  que $\sum\limits_{i=1}^{m} s_i = k$,
    todo elemento de $L$ tiene cardinalidad $k$. Buscamos ver que la
    subgr\'afica inducida de $F_k(G)$ por $L$, i.e., $F_k(G,L)$, es isomorfa a
    $F_{s_1}(H_1) \square \cdots \square F_{s_m}(H_m)$. Primero, analizamos el
