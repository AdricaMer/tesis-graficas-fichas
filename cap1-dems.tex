...
Este cap\'itulo se enfoca en la conexidad de las gr\'aficas de fichas. Establece
algunas cotas para el di\'ametro al igual que el n\'umero de trayectorias
internamente ajenas que se pueden encontrar en una gr\'afica de fichas,
bas\'andose en las caractere\'isticas y par\'ametros de las gr\'aficas de las
cu\'ales provienen.
...
\le 2k$. Luego, tenemos que $|A \triangle B| \leq 2k$, por lo que
...
$a\notin A'\cup B$ y $a\notin A\cap B$, pero $a\in A\cup B$. Por eso, tenemos
...
Ahora demostramos la cota inferior. Sabemos que $G$ es una gr\'afica conexa con
di\'ametro $d$, por lo que existen vertices que est\'an a distancia $d$, digamos
$x$ y $y$. Ahora, construimos una partici\'on de $V$ usando la distancia que
tiene cada v\'ertice a $x$. Es decir, para cada $i\in [0,d]$, sea $V_{i}$ el
conjunto de v\'ertices de $G$ a distancia $i$ de $x$. Por tanto, tenemos que
$V_{0}=\{x\}$ y $y\in V_{d}$. Denotamos $d_x(v)$ a la distancia entre $x$ y el
v\'ertice $v$.

Sea $a$ el m\'inimo \'indice para el cu\'al se tiene $k \leq |V_{0}\cup
V_{1}\cup \cdots \cup V_{a}|$ y sea $b$ el m\'aximo \'indice para el cu\'al se
tiene $k\leq |V_{b}\cup V_{b+1}\cup \cdots \cup V_{d}|$. Tomamos $A$ un
$k$-subconjunto de $V_{0}\cup \cdots \cup V_{a}$  tal que $A\subseteq V_{0}$ o
$V_{0}\cup \cdots \cup V_{a-1}\subseteq A$. Adem\'as, tomamos $B$ un
$k$-subconjunto de $V_{b}\cup \cdots \cup V_{d}$ tal que $B\subseteq V_{d}$ o
$V_{b+1}\cup \cdots \cup V_{d}$. 

Consideramos una trayectoria entre $A$ y $B$ en $F_{k}(G)$. Cualquier ficha
inicialmente en $A$, digamos $v \in G$, se mueve a alg\'un v\'ertice en $B$,
digamos $v'\in G$. Notamos que todas las aristas de $G$ est\'an dentro de
alg\'un $V_{i}$ o, a lo m\'as, entre alg\'un $V_{i}$ y $V_{i+1}$, con
$i\in[0,d]$. As\'i pues, para la ficha en $v$ se necesitan al menos
$d_x(v')-d_x(v)$ movimientos para llegar a $v'$, ocupando s\'olo las aristas
entre $V_{i}$ y $V_{i+1}$, $i\in [0,d]$. Por lo tanto, el di\'ametro de
$F_{k}(G)$ es al menos $\sum_{v\in A}(d_x(v')-d_x(v))= \sum_{w\in
B}d_x(w)-\sum_{v\in A}d_x(v)$. Observamos que, al ser $G$ conexa, toda $V_{i}$
tiene al menos un elemento y por construcci\'on $V_{i} \cap
V_{i+1}=\varnothing$, para toda $i\in [0,d]$. Tomamos el caso en el que
$|V_{i}|=1$, para toda $i\in [0,d]$. Luego, tenemos que $k\leq
|V_{b}\cup\cdots\cup V_{d}|=|V_{b}|+|V_{b+1}|+\cdots +|V_d|$ $=\sum_{b}^{d}1 = d
-b+1$. An\'alogamente, tenemos que $k\leq |V_{0}\cup V_{1}\cup \cdots \cup
V_{a}|=|V_{0}|+|V_{1}|+\cdots + |V_{a}|$ $=\sum_{0}^{a} 1 = a+1$. En ambos
casos, la cota m\'inima se alcanza en la igualdad, por lo que tomamos $a=k-1$ y
$b=d-k+1$. Por lo tanto, tenemos que el di\'ametro de $F_{k}(G)$ es al menos
$\sum_{j=d -k+1}^{d}j - \sum_{i=0}^{k-1}i = k(d-k+1)$.
\end{proof}
...
un vecino. \Cref{fig:ex-diamT} muestra el movimiento de las fichas en $F_4(G)$
...
\{1,2,3,4\}$. Adem\'as, cada movimiento corresponde a un v\'ertice en la
trayectoria en $F_4(G)$ que va de un extremo a otro. Por lo tanto, tenemos que
la trayectoria que va de $\{v_1,v_2,v_3,v_4\}$ a $\{w_1,w_2,w_3,w_4\}$ tiene
longitud $4 \cdot 6 = 24$.
...
\begin{proof}
    ...
    s_m \in \mathbb{Z}$ como en el enunciado. Definimos el conjunto $L$ de los
    $k$-subconjuntos de $G$ que cumplen tener exactamente $s_i$ elementos de
    $H_i$, para $i \in \{1, \dots, m\}$, es decir, $L = \{A \subseteq G \colon\
    |A\cap H_1|=s_1 , |A \cap H_2|=s_2, \dots, |A \cap H_m|=m \}$. Observamos
    que, debido a  que $\Sigma s_i = k$, todo elemento de $L$ tiene cardinalidad
    $k$. Buscamos ver que la subgr\'afica inducida de $F_k(G)$ por $L$, i.e.,
    $F_k(G,L)$, es isomorfa a $F_{s_1}(H_1) \square \cdots \square
    F_{s_m}(H_m)$. Primero, analizamos el conjunto de v\'ertices de ambas
    gr\'afcias de fichas. Sea $A \in V(F_k(G,L))$, de tal manera que $A=\{a_1,
    \dots, a_{s_1}, b_1,\dots, b_{s_2}, \dots, m_1, \dots, m_{s_m}\}$. Por
    definici\'on $|A \cap V(H_i)|= s_i$, para $i \in \{1, \dots, m\}$. Adem\'as,
    sabemos que cada v\'ertice de $F_{s_i}(H_i)$ tiene cardinalidad $s_i$, para
    $i \in \{1, \dots, m\}$, por lo que tenemos que $A \in V(H_1) \times \cdots
    \times V(H_m)$. De manera an\'aloga, si tomamos $A \in V(H_1) \times \cdots
    \times V(H_m)$, $A = \{A'_1, A'_2, \dots, A'_m\}$ con $A'_i \in
    F_{s_i}(H_i)$. De lo anterior, tenemos que $A \cap V(H_i) = A'_i$, por lo
    que $|A \cap V(H_i)|= s_i$, para toda $i \in \{1, \dots, m\}$.
    Adicionalmente, tenemos que $|A|= k$ puesto que $\Sigma s_i = k$. Por lo
    tanto, $A \in V(F_k(G,L))$.
    ...
    j$, por lo que $b, c \in H_i$. \'Esto significa que las fichas en
    $H_j$ est\'an est\'aticas, para todo $j \in \{1, \dots, m\} \setminus
    \{i\}$. Lo cu\'al pasa si y s\'olo s\'i $BC$ es una arista en la
...
una gr\'afica y su gr\'afica de $k$-fichas. Para esto, empezamos
...
los siguientes dos lemas nos son de utilidad.
...
\begin{proof}
    ...
    v_{2}, \dots, v_{p}\}$, con $v_{1} = a$. Notamos que, si $k > p$, entonces
    ...
    anterior, tenemos que  $|R \cap V(P)| = p$. Adem\'as, por construcci\'on de
    $A \xrightarrow[P]{} A'$, $R$ tiene una ficha en la trayectoria $(v_{p},b
    ]$. Por ello, $R$ no contiene al conjunto $\{v_{2}, \dots, v_{p}\}$. Por
    otro lado, como $\{v_{2}, \dots, v_{p}\}$ est\'a en $A \cap V(P)$, y $Q$ y
    $P$ son internamente ajenas, entonces $\{v_{2}, \dots, v_{p}\}$ est\'an
    est\'aticos en cada v\'ertice de $A \xrightarrow[Q]{} A'$. Por lo tanto,
    ...
    otra parte, ya que $P$ y $Q$ son internamente ajenos y s\'olo comparten el
...
\begin{proof}
    ...
    Ahora suponemos que $|Y|>1$. Como tenemos que $|Y|<|Z|$, y hay a lo m\'as
    ...
    es ac\'iclica y las aristas $\{vx\}$ y $e$, en caso de existir, no est\'an
    en $M'\cup R'$, entonces tenemos que $M \cup R$ es ac\'iclico.
\end{proof}
...
\textbf{X}},{\color{baige} \textbf Y})$. Sea ${\color{azulMetal}\bf M}$ el
conjunto de aristas azules mencionado en \cref{lem:rb-bipGraph}, resaltadas en
figura, tenemos que ${\color{azulMetal} \bf M=\{v_1v_8,
v_2v_9,v_3v_7,v_4v_{10}\}}$. Notamos que todo v\'ertice en ${\color{baige} \bf
Y}$ tiene exactamente una arista en ${\color{azulMetal}\bf M}$. Adem\'as, la
uni\'on de las aristas de ${\color{azulMetal}\bf M}$ y las aristas rojas, no
forma un ciclo.
...
\begin{proof}
    ...
    de $A \cap B$. Por ello, tenemos que $|A\cap B| = |C| + |D| = k-1$. Adem\'as
    ...
    \cap B$. Empezamos considerando $v \in C$, por lo que $v \in Q_i$ para un  $i
    \in \{1, \dots, s\}$. Definimos $Y_v = N_G(v) \setminus ((A \cap B) \cup
    ...
    incide en a lo m\'as una arista roja. Luego, notemos que $|\{1, \dots, l\}|
    = l  \geq t-k+ 1+ |D| > t-k + |D|$. Si $v \in D$, entonces $|D| \geq 1$ por
    lo que $l \geq t- k+1 = |Y_v '|$. Si $v \in C$, tenemos que $l \geq t-k =
    |Y_v '|$. As\'i, podemos usar \cref{lem:rb-bipGraph} en $H_v$ con $Z= \{1,
    \dots, l\}$ y $Y = Y_v '$. Por lo tanto, hay un conjunto $M_v$ de aristas
    azules tal que cada v\'ertice en $Y$ incide en exactamente una arista de
    $M_v$ y la uni\'on de aristas rojas y aristas de $M_v$ es ac\'iclica.
    Notamos que $|M_v|=|Y|$.
    ...
    $x=y'$ y $y=x'$. Por ello, tenemos que $x \in P_{i'}$ y $x'\in P_i$, lo cual
    ...
    Ahora pasamos a demostrar que los tipos de trayectorias formadas son
    internamente ajenas dos a dos. Observamos que las trayectorias de tipo $Q$ y
    de tipo $P$ son internamente ajenas por construcci\'on. As\'i pues,
    comenzamos con las trayectorias de tipo $R$ y las de tipo $P$. Sean
    $R\langle v,x \rangle$ y $A \xrightarrow[P_i]{}  B$, para alg\'un $i \in
    \{1, \dots, l\}$, trayectorias en $F_k(G)$ de tipo $R$ y $P$,
    respectivamente. Por construcci\'on, $v$ no est\'a en v\'ertice interno
    alguno de $R \langle v,x \rangle$. Por otro lado, como $ v \in A\cap B$,
    entonces $v$ est\'a est\'atico en $A \xrightarrow[P_i]{}  B$, es decir, $v$
    est\'a en cada v\'ertice de la trayectoria. Por lo tanto, tenemos que las
    trayectorias $R\langle v,x \rangle$ y $A \xrightarrow[P_i]{}  B$, para
    alg\'un $i \in \{1, \dots, l\}$, son internamente ajenas.

    Ahora nos enfocamos las trayectorias de tipo $Q$ y las de tipo $R$. Sean
    ...
    las trayectorias son internamente ajenas. Ahora consideramos el caso en el
    ...
    $v \in D$ hay $t-k+1$ trayectorias de tipo $R$. Por lo que en $F_k(G)$ hay $l+
...
${\color{baige}\bf A= \{1,2,3\}}$ y ${\color{baige}\bf B=\{2,3,5\}}$ los
v\'ertices en $F_3(G)$ sobre los que resaltamos los distintos tipos de
trayectorias internamente ajenas obtenidas en el lema anterior. Tenemos que
${\color{baige} \bf A\triangle B=\{1,5\}}$ y $A\cap B=\{2,3\}$, por lo que
usamos $15$-trayectorias para encontrar las trayectorias de tipo $P$ y las de
tipo $Q$, de manera que las de tipo $P$ no tienen a $2$ ni a $3$ como v\'ertice
interno y las de tipo $Q$ tienen a ambos como v\'ertices internos, recordando
que $A\cap B=\{2,3\}$. Tomamos ${\color{naranja}\bf P_1=(1,4,5)}$ y
${\color{fushia}\bf Q_1(1,2,3,5)}$. Ahora tomamos ${\color{fushia}\bf 2} \in C$
y su vecino ${\color{menta}\bf x\notin P_1}$, en este caso $x=6$. De esta manera
tenemos la trayectoria del tercer tipo
\[
...
las trayectorias internamente ajenas de ${\color{baige}\bf A}$ y
${\color{baige}\bf B}$.}
...
cotradicci\'on, pues tomamos a $A$ y $B$ en distintas componentes, lo que
...
Notamos que hay $2r^2$ de estos conjuntos. Supongamos que $A_{i,j} \notin
\mathcal{C}$, para algunos $ i, j \in \{1, \dots, r\}$, entonces tenemos que $A
\triangle A_{i,j} = \{a_i, b_j\}$. Por tanto, $A$ y $A_{i,j}$ est\'an en la
misma componente de $F_k(G)- \mathcal{C}$. Por otro lado, $|B \triangle A_{i,j}|
= 2 (r-1)$, por lo que $B$ y $A_{i,j}$ est\'an en la misma componente de $F_k(G)
- \mathcal{C}$. Por ende, tenemos que $A$ y $B$ est\'an en la misma componente
...
${\color{naranja}\bf \{1,3,5,6\}}$ y ${\color{fushia}\bf
\{\{1,2\},\{1,3\},\{1,5\},\{1,6\},\{2,4\},\{3,5\},\{3,6\},\{5,6\}\}}$ para $G$ y
$F_2(G)$, respectivamente. Adem\'as, utilizando \cref{fig:ex-tok-path}, podemos
ver que $F_3(G)$ es $9$-conexa. Por lo tanto, tenemos que $F_2(G)$ y $F_3(G)$
son gr\'aficas $4$-conexas.
