...
$k$-subconjunto de $V_{b}\cup \cdots \cup V_{d}$ tale que $B\subseteq V_{d}$ o
$V_{b+1}\cup \cdots \cup V_{d}$. 

Consideramos una trayectoria entre $A$ y $B$ en $F_{k}(G)$. Cualquier
ficha inicialmente en $A$, digamos $v \in G$, se mueve a
alg\'un v\'ertice en $B$, digamos $v'\in G$. Notamos que todas
las aristas de $G$ est\'an dentro de alg\'un $V_{i}$ o, a lo m\'as, entre alg\'un
$V_{i}$ y $V_{i+1}$, con $i\in[0,d]$. As\'i pues, para la ficha en $v$ se
necesitan al menos $d_x(v')-d_x(v)$ movimientos para llegar a $v'$, ocupando
s\'olo las aristas entre $V_{i}$ y $V_{i+1}$, $i\in [0,d]$. Por lo tanto, el
di\'ametro de $F_{k}(G)$ es al menos $\sum_{v\in A}(d_x(v')-d_x(v))= \sum_{w\in
...
la cota m\'inima se alcanza en la igualdad, por lo que tomamos $a=k-1$ y
...
Para ejemplificar este tipo de gr\'aficas, consideremos la trayectoria $P_6$.
Recordamos que $F_2(P_6) \cong F_4(P_6)$, $F_1(P_6) \cong F_5(P_6)$ y $F_6(P_6)
\cong K_1$, por lo que en \cref{fig:ex-diamP} se muestran las gr\'aficas de $2$ y
...
\{1,2,3,4\}$. Adem\'as, cada movimiento corresponde a un
...
\begin{proof}
    ...
    Buscamos ver que la subgr\'afica inducida de $F_k(G)$ por $L$, i.e., $F_k(G,L)$,
    ...
    Por definici\'on $|A \cap V(H_i)|= s_i$, para $i \in
    ...
    \cdots \square F_{s_m}(H_m))$.
     ...
     \cdots \square F_{s_m}(H_m)$ y $F_k(G,L)$, una subgr\'afica
     inducida de $F_k(G)$. Por ende, $F_{s_1}(H_1) \square
...
\begin{proof}
    ...
    Ahora, suponemos que $|A \cap V(P)| = 1$ y $|A \cap V(Q)| = 1$, es decir,
...
\begin{proof}
    ...
    digamos $e'$. Por tanto, la uni\'on de aristas rojas y aristas en $M$ es $\{e,
    e'\}$, que no es un ciclo.
    ...
    es ac\'iclica y las aristas $\{vx\}$ y $e$, en caso de existir, no
    est\'an en $M'\cup R'$, entonces tenemos que $M \cup R$ es ac\'iclica.
\end{proof}
...
\begin{proof}
    Sea $G$ una gr\'afica $t$-conexa y sean $A$ y $B$ v\'ertices de $F_{k}(G)$
    tales que $|A \triangle B| = 2$. Empezamos por probar %Inicamos probando
    que el n\'umero de $AB$-trayectorias internamente ajenas en $F_{k}(G)$ es
    al menos $t$. 
    ...
    respectivamente. Notamos que, por como est\'an definidas, toda $AB$-trayectoria
    ...
    de tipo $R$ son internamente ajenas por construcci\'on. As\'i pues,
...
trayectorias internamente ajenas obtenidas en el Lema. Tenemos que ${\color{baige} \bf A\triangle
...
tipo $P$ no tienen a $2$ ni a $3$ como v\'ertice interno y las de tipo $Q$
tienen a ambos como v\'ertices internos, recordando que $A\cap B=\{2,3\}$.
Tomamos ${\color{naranja}\bf P_1=(1,4,5)}$ y ${\color{fushia}\bf Q_1(1,2,3,5)}$.
Ahora tomamos ${\color{fushia}\bf 2} \in C$ y su vecino ${\color{menta}\bf
x\notin P_1}$, en este caso $x=6$. De esta manera tenemos la trayectoria del
tercer tipo
\[
...
\triangle A_{i,j} = \{a_i, b_j\}$. Por tanto, $A$ y $A_{i,j}$ est\'an en la misma
...
\cref{teo:FG-t-conexa}. Nos fijamos que $G$, mostrada del lado izquierdo, es
una gr\'afica $4$-conexa y su gr\'afica de $2$-fichas, mostrada del lado
derecho, es $8$-conexa. En la figura, se muestran los conjuntos de corte
${\color{naranja}\bf \{1,3,5,6\}}$ y ${\color{fushia}\bf
\{\{1,2\},\{1,3\},\{1,5\},\{1,6\},\{2,4\},\{3,5\},\{3,6\},\{5,6\}\}}$ para $G$ y
$F_2(G)$, respectivamente. Adem\'as, utilizando \cref{fig:ex-tok-path}, podeos
...
\begin{teorema}%
    ...
    \begin{proof}
        ...
        que $-2k+4 \ge -k^2 + k$, lo cu\'al implica que $kt -2k +4 \ge kt - k^2
        + k = k (t - k +1)$. Luego, tenemos que $|\mathcal{C}| \ge tk -2k +4
        \ge k(t-k+1)$. Por lo tanto, $F_k(G)$ es $k(t-k+1)$-conexa.
    \end{proof}