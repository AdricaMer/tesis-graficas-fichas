...
bas\'andonos en las caractere\'isticas y par\'ametros de las gr\'aficas de
cuales provienen.
...
Es interesante notar que los l\'imites obtenidos en \cref{teo:diamFG} son
alcanzables. A continuaci\'on se muestran unos ejemplos para dichas cotas. La
cota inferior se cumple para toda trayectoria de $d +1$ v\'ertices, con $k
\leq d +1$, es decir una trayectoria de di\'ametro $d$. Es f\'acil
observar que para toda $k$ obtenemso una gr\'afica de $k$-fichas de di\'ametro
$k(d-k+1)$. La Figura \ref{fig:ex-diamP} muestra la trayectoria $P_6$ con todas sus
posibles gr\'aficas de fichas, con $k<6$. Recordamos que $F_2(P_6) \cong
F_4(P_6)$ y $F_1(P_6) \cong F_5(P_6)$. %Falta P_6 en la figura, donde se pone?
...
$G$, obtenida de a\~{n}adirle $k$ v\'ertices cada extremo de la trayectoria
...
$k$-fichas de $G$ tiene di\'ametro $dk$. Para enfatizar esto,
\cref{fig:ex-diamT} muestra una gr\'afica $G$ formada de tomar la trayectoria
$P_5$ y tomando $k=4$. Notamos que los v\'ertices $\{v_1,v_2,v_3,v_4\}$ y
$\{w_1,w_2,w_3,w_4\}$ son extremos en $F_4(G)$ pues ambos tienen \'unicamente un
vecino. La Figura \ref{fig:ex-diamT} muestra el movimiento de las fichas en
$F_4(G)$ de un extremo al otro extremo. Observamos que una ficha en alg\'un
$v_i$ tiene que moverse por $d +1$ ......alg\'un $w_i$ es , para $i \in
...
$\{w_1,w_2,w_3,w_4\}$ longitud $4(6)=24$.
...
resaltando el movimiento de las $4$ fichas al pasar de un extremo de la gr\'afica al otro}
\label{fig:ex-diamT}       
\end{figure}
...
%Falta corolario del articulo!!!

\section{Trayectorias Internamente Ajenas}%
\label{sec:TrayIntAj}

El objetivo de este cap\'itulo es estudiar la relac\'on de la conexidad entre
una gr\'afica y su gr\'afica de $k$-fichas. Para esto, primero nos enfocaremos
en determinar la cantidad de trayectorias internamente ajenas en una gr\'afica
de $k$-fichas. Veremos que podemos esta cantidad esta relacionada con la
conexidad de la gr\'afica original. Adem\'as, para gr\'aficas con conexidad
suficientemente grandes, podemos encontrar un n\'umero mayor de trayectorias
internamente ajenas. Para llegar a estos resultados, primero se necesitan
demostrar dos lemas que nos ser\'an de utilidad.
...
Seguidamente se presenta el siguiente lema t\'ecnico, que ser\'a necesario para
demostrar el resultado prinsipal de esta secci\'on. 
...
Para aclarar \cref{lem:rb-bipGraph}, m\'as adelante \cref{fig:ex-rb-bioGraph}
muestra una gr\'afica bipartita con bipartici\'on $({\color{crema}
...
${\color{azulMetal}M}$. Adem\, la uni\'on de las aristas de $M$ y las aristas
rojas, no forma un ciclo.
...
\cref{lem:TrayIntAj}, \cref{fig:ex-tok-path}, a continuaci\'on mostrada, muestra
una gr\'afica $G$ del lado izquierdo y su gr\'afica de $3$-fichas del lado
izquierdo. Ahora construimos una trayectoria de cada tipo. Consideramos
...
Tenemos que ${\color{baige}A\triangle B=\{1,5\}}$ y $A\cap B=\{2,3\}$ por lo que
usamos $15$-trayectorias para encontrar las trayectorias de tipo $P$ y las de
tipo $Q$, de manera que las de tipo $P$ no tiene a $2$ ni a $3$ como v\'ertice
interno y las de tipo $Q$ tienen a ambos como v\'ertices internos, recordando
que $A\cap B=\{2,3\}$. Tomamos ${\color{naranja}P_1=(1,4,5)}$ y
...
Pasamos a enfocarnos en la conexidad de las gr\'aficas de fichs. Con ayuda de
los resuldados encontrados en las secciones anteriores, esta secci\'on buscamos
estudiar la relaci\'on entre la conexidad de una gr\'afica y su gr\'afica de
$k$-fichas. 

El Teorema a continuaci\'on mostrado nos muestra la relaci\'on buscada.
...
\cref{fig:ex-tConect}. Nos fijamos en que $G$, mostrada del lado izquierdo, es
una gr\'afica $4$-conexa y su gr\'afica de $2$-fichas, mostrada del lado
derecho, es $8$-conexa. En la figura se muestra los conjuntos de corte
${\color{naranja}\{1,3,5,6\}}$ y
${\color{fushia}\{\{1,2\},\{1,3\},\{1,5\},\{1,6\},\{2,4\},\{3,5\},\{3,6\},\{5,6\}\}}$
para $G$ y $F_2(G)$ respectivamente. Adem\'as, utilizando
\cref{fig:ex-tok-path}, podeos ver que $F_3(G)$ es $9$-conexa. Por lo tanto tenemos
que $F_2(G)$ y $F_3(G)$ son gr\'aficas $4$-conexas.


Por \'ultimo, el siguiente teorema presenta una cota m\'as (fuerte?/mayor) para
