...
La \textbf{conexidad} \indiceSub{conexidad}{local} entre dos v\'ertices
...
gr\'afica no trivial es \indiceSub{gr\'afica}{$t$-conexa} si, para cualesquiera
...
V(H) \}$. En \cref{fig:ex-unionCompleta} se muestra, del lado izquierdo, un
...
gr\'afica $G$ y querer colocar $k$ fichas indistinguibles sobre los v\'ertices
de esta gr\'afica, a lo m\'as una ficha por v\'ertice. Una ficha se puede
"mover" de un v\'ertice de $G$ a otro siempre y cuando exista una arista entre
ellos y no haya una ficha en el segundo v\'ertice. Cada configuraci\'on de $k$
fichas ser\'a un v\'ertice en nuestra nueva gr\'afica donde dos v\'ertices
ser\'an adyacentes siempre que se puedea llegar de una configuraci\'on a otra al
mover una ficha atrav\'es de una arista. A esta nueva gr\'afica la nombramos la
gr\'afica de $k$-fichas de $G$ y la denotamos $F_k(G)$. Formalizando este
concepto tenemos que, dada una gr\'afica $G$ y $k$ un entero positivo, la
...
\indiceSub{gr\'afica}{$k$-fichas} \textbf{de $G$} \index{fichas!gr\'afica}. En
\cref{fig:ex-tok-path} se muestra un ejemplo de una gr\'afica $G$ y su gr\'afica
de $3$-fichas.
...
Teniendo eso en mente, definimos el siguiente concepto. Sea $P$ una
$ab$-trayectoria en la gr\'afica $G$, tomamos $A$ un $k$-conjunto en $G$ de tal
manera que $a\in A$ y $b \notin A$ y definimos \indiceSub{fichas}{$A'$} como
$A'= (A \setminus \{a\}) \cup \{b\}$. Si $A\cap P =\{v_1, \dots, v_q\}$, con
$v_1 = a$, definimos \indiceSub{fichas}{$A \xrightarrow[P]{} A'$} como la
trayectoria en $F_k(G)$ entre $A$ y $A'$ que sigue la siguiente secuencia.
Primero movemos la ficha de $v_q$, v\'ertice de $G$, hacia el v\'ertice $b$,
v\'ertice de $G$, por $P$. Luego, para los v\'ertices $v_i$ en $G$,con $i \in
\{q-1, q-2, \dots 1\}$, movemos la ficha de $v_i$ a $v_{i+1}$. A lo largo de
este trabajo nos ser\'a de utilidad $A'$ por lo que .....