...
\indiceSub{gr\'afica}{orden} de $G$ y los elementos de $V(G)$ son llamados
\indice{v\'ertices}. Los elementos de $E(G)$ son \indice{aristas}. Para una
...
\indiceSub{subgr\'afica}{generadora}. Otro concepto que se deriva de las
...
\textbf{subgr\'afica} \indiceSub{subgr\'afica}{inducida} de $G$ por $S$ y la
...
unitario, i.e., el caso en el que le quitamos un solo v\'ertice $v$ a $G$.
...
\textbf{ciclo}\indiceSub{ciclo}{generador} es un ciclo cuyo conjunto de
...
se le llama \indiceSub{corte por v\'ertices}{$uv$-corte}\index{v\'ertices!$uv$-corte} \textbf{por
...
$X,Y \subseteq V(G)$, $S$ es un \indiceSub{corte por v\'ertices}{$XY$-corte
...
\begin{proof}
    ...
    por lo que se cumple la hip\'otesis de inducci\'on, i.e., la familia de
...
\begin{proof}
    ...
    generadora de $G \oplus G'$. Observamos que $V(H)=V(G)$ y $V(H')=V(G)$, por
    lo que $V(H \oplus H')=V(G \oplus G')$. Por consiguiente, basta ver que las
    aristas de $H \oplus H'$ son aristas de $G \oplus G'$. Sean $x$ y $y$
    ...
    si $x \in V(H)$ y $y \in V(H')$, por definici\'on de uni\'on completa
    tenemos que $x \sim y$ en $G \oplus G'$. De este modo tenemos que $H \oplus
    H'$ es una subgr\'afica generadora de $G \oplus G'$.
\end{proof}
...
$\{{\color{azulCielo}\bf{v_1}}, {\color{coral}\bf{v_2}}\}$ y la segunda en la parte
inferior y con conjunto de v\'ertices
$\{{\color{amarillo}\bf{w_1}},{\color{morado}\bf{w_2}}\}$. Ambas gr\'aficas son copias de
...
conoce como la gr\'afica \indice{trivial}. Algo que es importante de mencionar
...
Una familia de gr\'aficas, muy estudiada en Teor\'ia de Gr\'aficas, es la
familia de las gr\'aficas $k$-partitas. Antes de definir esta familia, es
importante se\~{n}alar que una
\indiceSub{v\'ertices}{partici\'on}\index{partici\'on} de $V(G)$ en $V_1, V_2,
\dots, V_k$ partes debe satisfacer $V_1 \cap V_2 \cap \cdots \cap V_k =
\varnothing$ y $V_1 \cup V_2 \cup \cdots \cup V_k = V(G)$, sin embargo, no
pedimos que cada $V_i$ sea no vac\'io, para $i \in \{1, \dots,k\}$. Con \'esto
pasamos a definir la familia de gr\'aficas. Una gr\'afica es
\indiceSub{gr\'afica}{$k$-partita} si su conjunto de v\'ertices admite una
...
\begin{proof}
    ...
    partici\'on, para cada $G_i$. Primero tenemos que $X_i \cap Y_i =
    ...
    Ahora nos enfocamos en la segunda implicaci\'on, i.e., si $G_i$ es una
    gr\'afica bipartita, para $i \in \{1, \dots, n\}$, entonces $G$ tambi\'en es
    bipartita. Proponemos la bipartici\'on $(X,Y)$ para $G$, donde $X = \bigcup
    X_i$ y $Y =\bigcup Y_i$. Nos fijamos en que $X \cup Y = \bigcup
    V(G_i)=V(G)$. An\'alogamente tenemos que $X \cap Y = \varnothing$. Por
    \'ultimo, dados $x \in X$ y $y \in Y$, tenemos que $x \in X_i$ y $y \in
    Y_j$, para algunos $i,j \in \{1, \dots,n\}$. Si $i \neq j$, entonces los
    v\'ertices estan en distintas componentes conexas, por lo que no son
...
\indice{abanico} $\mathcal{F}_n$ es la gr\'afica obtenida de la uni\'on
...
\indiceSub{abanico}{generalizado}, denotiado $\mathcal{F}_{m.n}$, se define como
...
trayectoria generadora, i.e., una trayectoria cuyo conjunto de v\'ertices es
...
\indiceSub{gr\'afica}{$k$-crom\'atica}. En \cref{fig:ex-color-prop}, se una
$3$-coloraci\'on propia por v\'ertices, sin embargo el n\'umero crom\'atico de
la gr\'afica es $2$. 
...
color. Adem\'as, tambi\'en podemos determinar como se ven las gr\'aficas
...
entonces $\chi(H) \leq \chi(G)$; m\'as a\'un, si si $H$ es una gr\'afica
