...
Al conjunto de vecinos de un v\'ertice $v$ se le llama \indice{vecindad}, y se
denota $N_G(v)$, mientras que al n\'umero de aristas incidentes en $v$ se le
llama \indice{grado} de $v$, se denota $d(v)$. Un \textbf{v\'ertice}
\indiceSub{v\'ertices}{aislado}, es decir, un v\'ertice que no tiene vecinos,
tiene grado $0$.
...
ambos diagramas de $G$, los v\'ertices tienen las mismas propiedades, por
ejemplo, que $N_G(v_3)=\{v_1,v_4,v_5\}$ o que el grado de $v_2$ es $1$. \'Esto
se debe a que la gr\'afica es un objeto matem\'atico, independiente del
diagrama.
...
\label{sec:clanes-CIndep}
    ...
    n\'umero de elementos de {clan}una gr\'afica es el \indice{n\'umero de clan}
    y se denota $\omega$. En \cref{fig:ClanInd} se muestra un ejemplo de un
...
\indiceSub{camino}{longitud}, el n\'umero de posiciones en $W$ que son ocupadas
...
\indice{distancia} entre dos v\'ertices $u$ y $v$ de una gr\'afica, denotada
$d(u,v)$, como la longitud de la $uv$-trayectoria m\'as corta. Sea $P$ una
trayectoria en una gr\'afica, decimos que $P$ no tiene \indice{cuerdas} si la
...
definir su \indice{di\'ametro} como $\max_{v\in G}\{\max_{u\in G}\{d(u,v)\}\}$. 
...
su \textbf{trayectoria} \indiceSub{trayectoria}{inversa}, a la cu\'al denotamos
...
donde, para mostrar una trayectoria contenida, se resaltan los v\'ertices
de${\color{morado}\textbf{morado}}$ de la $v_3 v_5$-trayectoria contenida.
...
una gr\'afica. Para \'sto puede resultar natural cuestionarse si existe alg\'un
...
V(G)$, $S$ es un \indiceSub{cortepor
v\'ertices}{$XY$-corte}\index{v\'ertices!$XY$-corte} \textbf{por v\'ertices} si
...
Ahora, usando el concepto de corte por v\'ertices, definimos la conexidad de una
gr\'afica. El \indice{n\'umero de conexidad} de una gr\'afica $G$, denotado por
$\kappa(G)$, se define como la m\'inima cardinalidad de un conjunto de corte de
$G$, es decir, $\kappa(G)=\min \{|S| \colon\ S \textnormal{ es corte por
v\'ertices}\}$. Como ya se observ\'o, las gr\'aficas completas no tienen cortes
por v\'ertices, no obstante, por convenci\'on decimos que una gr\'afica completa
...
gr\'afica inconexa tiene n\'umero de conexidad $0$. Adicionalmente, decimos que
una gr\'afica $G$ es \indiceSub{gr\'afica}{$t$-conexa} si $|G|>t$ y $G-X$ es
conexa para todo conjunto de v\'ertices $X$ tal que $|X|<t$. Para ejemplificar
el conecpto de conexidad nos fijamos en las gr\'aficas en \cref{fig:isoGraf} y
