...
\indice{aristas}. Para una arista $e$ y v\'ertices $u, v$ de $G$, decimos que
...
dibujar a $G$ como la gr\'afica $H$ y viceversa, usando como gu\'ia los colores
que se muestran en la figura. De manera intuitiva, decimos que dos gr\'aficas
que comparten un mismo diagrama son ``iguales'' pues, al compartir un diagrama,
comparten muchas propiedades como el n\'umero de v\'ertices, las adyacencias o
el grado de los v\'ertices. Estas g\'aficas se dice que son isomorfas.
Formalmente hablando, dos gr\'aficas $G$ y $H$ son
...
gr\'aficas, nos referimos a subgr\'aficas. Si $G$ y $H$ son gr\'aficas tales que
...
\label{sec:clanes-CIndep}
    ...
    caso, el grado de todo v\'ertice de $S$ es $|S-1|$ en $G[S]$. De manera
    an\'aloga a los conjuntos independientes, la cardinaliad del clan con mayor
    n\'umero de elementos de una gr\'afica es el \indice{n\'umero de clan} y se
    denota $\omega$. En \cref{fig:ClanInd} se muestra un ejemplo de un
    ${\color{coral}\bf clan}$ y un ${\color{verde}\bf conjunto}$
    ${\color{verde}\bf independiente}$ de una gr\'afica. Es f\'acil observar que
    el ${\color{verde}\bf conjunto}$ ${\color{verde}\bf independiente}$ no es el
...
\{ 1, \dots, k\}$. Este $uv$-camino es una forma de vincular el v\'ertice $u$
con el v\'ertice $v$. En caso de que no sea necesario especificar de qu\'e
v\'ertice a qu\'e v\'ertice va la sucesi\'on, nos referimos simplemente a un
\indice{camino}. Es importante notar que un camino no siempre tiene aristas. Un
camino es \indiceSub{camino}{trivial} si tiene \'unicamente un v\'ertice y no
tiene aristas. \'Este es el \'unico caso en el que un camino no tiene aristas.
Al ser una sucesi\'on, a un camino $W$ se le puede asociar una
\indiceSub{camino}{longitud}, que es el n\'umero de posiciones en la sucesi\'on
$W$ que son ocupadas por aristas, se denota $\ell(W)$. 
...
longitudes entre dos v\'ertices. Definimos la \indice{distancia} entre dos
v\'ertices $u$ y $v$ de una gr\'afica, denotada $d(u,v)$, como la longitud de la
$uv$-trayectoria m\'as corta. Sea $P$ una trayectoria en una gr\'afica, decimos
que $P$ no tiene \indice{cuerdas} si la subgr\'afica inducida por $V(P)$ tiene
...
``tama\~{n}o'' a una gr\'afica $G$ al definir su \indice{di\'ametro} como
...
\begin{proof}
    ...
    que no es una trayectoria. \'Esto pues, en caso de que lo fuera, $W$ ser\'ia
    la trayectoria buscada. Tomamos $W= (w_0,e_1,w_1,e_2, \dots, e_n,w_n)$, con
    ...
    que $w_m = w_l$. Formamos el camino $W'= (w_0,e_1,w_1,\dots w_m, e_{l+1},
    w_{l+1} \dots, e_n,w_n)$. Observamos que $\ell(W')<\ell(W)$, por lo que, por
...
mostrar una trayectoria contenida, se resaltan los
...
\begin{proposicion}
    Sen  $S \subset V(G)$ un conjunto, entonces se cumple la siguiente
    desigualdad
    \[
...
\begin{proof}
    Sean $G$ una gr\'afica y $S \subseteq V(G)$. Sean $k,m,n \in \mathbb{Z^+}$
    tales que $\chi (G) =k$, $\chi (G[S]) =m$ y $\chi (G-S) =n$. Supongamos que
    existe $S \subseteq V(G)$ tal que $ \chi(G[S])+\chi(G-S) <\chi(G)$.
    Consideramos la coloraci\'on $c_1$ de $G[S]$ y la coloraci\'on $c_2$ de
    $G-S$. Tomamos la coloraci\'on $c$ en $G$ utilizando los colores $\{1, 2,
    \dots, m, m+1, \dots, m+n, \dots, k\}$ y asignandole los mismos colores que
    en $c_1$ y$c_2$, de ser posible. Dado que $k>m+n$, existe un v\'ertice $v
    \in V(G[S])$ tal que $N(v) \not \subset V(G[S])$ y $c_1 (v) = i$ y que $c(v)
    = j$, donde $i \in \{1, \dots, m\}$ y $j \in \{m+n+1, \dots, k\}$. En otras
    palabras, alg\'un veino $w \in V(G)$ de $v$ tiene color $i$. Esto pasa si y
    s\'olo si $w \in V (G[S])$, es decir, $w$ tambi\'en es vecino de $v$ en
    $G[S]$. Por ello, $c_1$ les asigna distintos colores, por lo tanto $c(v)
    \neq c(w)$.
\end{proof}
