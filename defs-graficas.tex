...
$V(G)$ tal que $G[V(G)-S]$ es inconexa. Decimos que $S$ es un \indice{$k$-corte
por v\'ertices} si $S|=k$. Cabe notar que no todas las gr\'aficas tienen alg\'un
...
conexidad de una gr\'afica. La \indice{conexidad} de una gr\'afica $G$, denotada
...
gr\'afica $G$ es \indiceSub{gr\'afica}{$t$-conexa} para toda $t \in \mathbb{N}$
