...
%comunmente
   ...
   $v_3$ y $v_5$ es $2$.
    ...
    \indiceSub{gr\'afica}{ac\'iclica}.
   
Hasta ahora nos hemos enfocado en los ``vinculos'' entre dos v\'ertices de una
...
\indiceSub{gr\'afica}{conexa} si, para cualquier par de v\'ertices $u,v \in
V(G)$, existe una $uv$-trayectoria. Si una gr\'afica no cumple esta propiedad,
decimos que es \indiceSub{gr\'afica}{inconexa}. Observamos que toda gr\'afica,
...
m\'aximas con la propiedad de ser conexas son llamadas \indice{componentes
conexas}. Notamos que si una gr\'afica es conexa, contiene s\'olo una componente
conexa, ella misma. 
...
un \indice{corte por v\'ertices} $v$ de una gr\'afica $G$ es un subconjunto de
...
entre dichos v\'ertices. La \indice{conexidad local} entre dos v\'ertices
...
Adicionalmente, definimos la \indice{conexidad} de una gr\'afica como el
...
Hay varias familias relevantes en la Teor\'ia de Gr\'aficas, en esta
    ...
    \indiceSub{gr\'afica}{bipartita completa} si es bipartita y dada la
    ...
    Una gr\'afica que no contiene ciclos se dice que es una gr\'afica
        ac\'iclica.
    \end{enumerate}
