...
\begin{teorema}
 ...
 \begin{proof}
     ...
     ajenas sea $t$, con $t<k$. Por el Teorema de Menger (referencia) tenemos
...
\begin{proof}
    ...
    tenemos que $v_k \in Y$ pues $v_kv_1 \in E(G)$ y $v_1 \in X$. Por lo tanto
    ...
    conjunto de la partici\'on como $Y = V(G)\setminus X$. Basta verificar que
    no existen aristas entre cualesquiera dos v\'ertices de $X$ o entre
    cualesquiera dos v\'ertices de $Y$. Supongamos que existen $u,w \in X$ tales
    que $uw \in E(G)$, entonces el ciclo $C=(v, \dots,u,w,\dots,v)$ tiene
    longiutd impar, lo cu\'al es una contradicci\'on. Analogamente, si
    consideramos $u,w \in Y$ con $uw \in E(G)$ obtenemos un ciclo impar. Por lo
    tanto $G$ es una gr\'afica bipartita con partici\'on $(X,Y)$.
\end{proof}
