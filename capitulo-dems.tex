...
\begin{proof}
    ...
    longitud total m\'i{}nima, por lo que  $\mathcal{P}$ no teine cuerdas.
    Definimos $P_{1}, \dots, P_{l}$ como las trayectorias en $\mathcal{P}$ que
    no intersectan $A \cap B$ y $Q_{1}, \dots, Q_{s}$ las
    trayectorias en $\mathcal{P}$ que intersectan a $A \cap B$. Entonces $l + s
    = |\mathcal{P}| > t$.

    Definimos como $C$ al conjunto de v\'ertices en $A \cap B$ que intersectan
    ...
    \xrightarrow[Q_l]{}  B$. Denotamos a estos tipos de $AB$-trayectorias como
    trayectorias de tipo P y de tipo Q respectivamente. Notamos que, por como se
    defini\'o, toda $AB$-trayectoria de tipo P no pasa por $A\cap B$, por lo que
    cada trayectoria de este tipo corresponde a la sequencia de fichas obtenidas
    al mover la ficha de $a$ a trav\'es de $P_i$ hacia b, con $i \in \{1, \dots,
    s\}$.
...
Ahora nos fijamos en que $A_{i,x} \triangle B_{j,x} = (A \triangle B) \setminus
...
$B$ est\'an en la misma componente de $F_k(G)- \mathcal{C}$. Pero esto es una
cotradicci\'on pues tomamos a $A$ y $B$ en distintas componentes. Esto implica
...
$\{B_j,x | j \in \{1, \dots, r\}\}$. ESto nos da $r(n-k-r)$ v\'ertices en
$\mathcal{C}$, pues $|A\cup B|=k +r$.

AHora definimos $A_i,j = (A\setminus \{a_i\}) \cup \{b_j\}$ y $B_i,j =
(B\setminus \{b_i\}) \cup \{a_j\}$, para toda $ i, j \in \{1, \dots, r\}$.
Notamos que hay $2r^2$ de estos conjuntos. Ahora supongamos que $A_i,j \notin
\mathcal{C}$, para algunos $ i, j \in \{1, \dots, r\}$. Entonces tenemos que $A
\triangle A_i,j = \{a_i, b_j\}$. Entonces $A$ y $A_i,j$ est\'an en la misma
componente de $F_k(G)- \mathcal{C}$. Por otro lado, $|B \triangle A_i,j | = 2
(r-1)$, por lo que $B$ y $A_i,j$ est\'an en la misma componente de $F_k(G) -
...
caso anterior. Por lo tanto tenemos que $A_i,j \in \mathcal{C}$. An\'alogamente
tenemos que $B_i,j \in \mathcal{C}$. 
