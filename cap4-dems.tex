...
\begin{figure}[ht!]
    ...
    \end{tikzpicture}
    \caption{En la parte, superior una gr\'afica hamiltoniana $G$
    (izquierda) y $F_2(G)$ (derecha).
    En la parte inferior una gr\'afica no hamiltoniana $K$ (izquierda)
    y $F_2(K)$ (derecha)}
    \label{fig:ex-H-NH}
\end{figure}
...
\begin{proof}
    ...
    $\{v_{n-k}, v_{n-k+1}, \dots, v_{n-2}, v_n\}$ y $\{v_{n-k}, v_{n-k+1},
    \dots, v_{n-2}, v_{n-1}\}$ son adyacentes. Por ello, asumios $k \geq 2$ para
    el resto de la demostraci\'on. Primero, consideramos el caso en el que $k
    =2$. Si tomamos $n = 3$, tenemos la gr\'afica con conjunto de v\'ertices
    ...
    Ahora veamos el caso de la gr\'afica de $2$-fichas de
    ...
    Nombramos $D_j =\{v_{n-k}, v_{n-k+1}, \dots, v_{n-1}, v_n\} \setminus
    ...
    $n-k+2 \leq j \leq n-2$. De \'esta manera obtenemos la trayectoria $Q= (D_n,
    D_{n-1}, \dots, D_{n-k+1})$ en $S_{n-k}$.     
...
\begin{proof}
    Comenzamos viendo que, para $n \geq 2$ y $1 \leq m \leq 2n$, la gr\'afica de
    $2$-fichas de $\mathcal{F}_{m,n}$ es hamiltoniana. Para esto construimos un
    ciclo hamiltorniano espec\'ifico para cada caso. Consideramos $m=1$ para el
    primer caso. Si $n=2$, entonces $F_2(\mathcal{F}_{m,n}) \cong
    ...
    partici\'on de $V(F_2(\mathcal{F}_{m,n}))$. %especificar mas?
    A continuaci\'on analizamos el siguiente ciclo $C$.
    ...
    $P_1$, i.e. $\{v_1,v_n\}$, que es adyacente al primer elemento de $P_2$, que
    ...
    $V(F_2(\mathcal{F}_{m,n}))$. Por lo tanto, $C_1$ es un ciclo hamiltoniano.
     \Cref{fig:exe-tray-n-2n} muestra el movimiento de fichas en la
...
Por \"ultimo, veamos el caso cuando $m>2n$. Sea $S \subset V(F_2(\mathcal{F}_{m,n}))$,
...
\begin{proof}
    ...
    $k=2$ usamos \cref{teo:2-TokGenerFan}. Basta ver que el ciclo obtenido en
